...
if __name__ == "__main__":
    ...
    prob_result_list_mae = ['mae_models/' + i for i in os.listdir('mae_models')]
    print("PROB RESULTS LIST BASE:", prob_result_list)
    print("PROB RESULTS LIST MAE:", prob_result_list_mae)
    ...
    prob_result_pkl_list_mae = []
    for i, prob_result_file in enumerate(prob_result_list_mae):
        print(i)
        with open(prob_result_file, 'rb') as f:
            prob_result_pkl_list_mae.append(pickle.load(f))

    len_results = len(prob_result_pkl_list)
    print(len_results)
    len_results_mae = len(prob_result_pkl_list_mae)
    print(len_results_mae)
    ...
    for feature, filenames, durations in tqdm(test_dataloader):
        ...
        out_mae = torch.zeros([len(feature), FEATURE_LEN]).to(device)
        with torch.no_grad():
            for prob_result_pkl in prob_result_pkl_list:
                out += torch.cat([prob_result_pkl[filename] for filename in filenames]) # [B, L]
            out /= len_results
            out = out * 0.77
            for prob_result_pkl in prob_result_pkl_list_mae:
                out_mae += torch.stack([torch.from_numpy(prob_result_pkl[filename]) for filename in filenames]).to(device)
            out_mae /= len_results_mae
            out_mae = out_mae * (1 - 0.77)
            out = out + out_mae
